...
| ステータス | **実装済み**（branch `stage2-cli`）。§9 の論点は 2026-09-24 に決定済み、実装で判明したことは §11、追加要望は §12 |
| 作成日 | 2026-09-24 |
...
## 12. 追加要望（2026-09-24、実装後にユーザーから）

### 12.1 既定の出力先

`--output` 省略時の出力先を **`output/<ジャンル>_<シード>.<拡張子>`**（カレントディレクトリの `output` フォルダ）に変更した
（従来はジャンルごとのフォルダ `<ジャンル>/<ジャンル>_<シード>.<拡張子>`）。フォルダが無ければ作る。`.gitignore` に `/output/` を追加。

### 12.2 表示言語（既定は日本語、`-e` / `--english` で英語）

| 対象 | 日本語（既定） | 英語（`-e`） |
|:---|:---|:---|
| usage（`--help`・引数なし） | 説明・オプションの説明・見出し（`使い方:`／`オプション:`／`ジャンル一覧:`）すべて日本語 | 英語（`usage:`／`options:`／`genres:`） |
| `--list-genres`・usage 末尾のジャンル一覧 | `description`（`別名:`） | `description_en`（`alias:`） |
| 実行結果のバナー | 見出し（`ジャンル`・`出力形式`・`シード`・`テンポ`・`出力ファイル`）と成功メッセージ、`(ランダム)`・`(指定 80-100)` | 従来どおり |

- **変えないもの**: ジャンルが出す要約行（`plan.summary`。コード進行名などの音楽用語）、`--version` の出力、
  標準エラー出力のエラー・警告（ライブラリ層の例外メッセージは英語のまま）、生成される曲。
- **ジャンルの英語説明**: `GenreProfile.description_en`（1行）を追加し、`register_profile` で `description` と同じ検査をする。
  新しいジャンルは日本語・英語の両方の説明が必須。
- **`-e` の判定**: usage の言語は引数解析の前に決める必要があるので、`-e`／`--english`（`--eng` などの省略形を含む）を先に探す。
  `-es 5` のようにまとめて書いた場合は解析後の値で英語にする（この書き方で usage を出すときだけ日本語になる）。
  `-e` だけを指定した場合は「引数なし」とみなし、英語の usage を出して終了する。
- **折り返し**: argparse は `len()` で折り返すので、全角文字が2桁ぶんはみ出す。ヘルプの折り返しを表示幅（全角=2桁）で行い、
  英数字の語（`free-jazz,` 等）は分割せず、句読点・閉じ括弧は行頭に置かない。
- **バナーの桁揃え**: 見出しを表示幅で 12 桁に揃える（`ジャンル    : ` と `Genre       : ` の `:` が同じ桁）。
