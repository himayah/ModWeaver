...
| FR-9 | ジャンル一覧と生成結果を機械向けの JSON でも出せる（`--json`。§8.8） |
| FR-10 | CLI の生成機能を GUI からも使える（§12）。GUI はジャンルなどの選択肢を起動時に CLI から受け取り、自分では持たない |
...
4. それ以外は生成（§2.3）。結果はバナー（§8.6）、`--json` があれば結果の JSON（§8.8）。
...
- 「作った曲」: その回に作った曲の一覧（新しい順。ランダムに選ばれたジャンルは名前に ` *` を付ける）。再生（OS の関連付け。関連付けが無ければ OpenMPT などの案内）、フォルダで表示、再現コマンドのコピー、**別の形式でも書き出す**（同じ seed で、指定があったテンポ・チャンネル数だけ渡す＝同じ曲）、**設定に読み込む**（seed を固定してテンポや形式だけ変える等）。選んだ曲の要約行・パス・再現コマンドを下に出す。
