...
- **6つの出力形式（`--format`）**:
  既定は Amiga ProTracker 4ch MOD（`M.K.`）。6ch・8ch のジャンル（例: `pop` の 6ch、`orchestral`・`cinematic`・`trailer` の 8ch）は FastTracker 系の多チャンネル MOD（`6CHN`・`8CHN`）になります。ほかに `.xm`／`.s3m`／`.it`（トラッカー形式）、General MIDI の `.mid`（DAW や GM 音源で鳴らせる）、`.mp3`（ffmpeg で音声化）を選べます。トラッカー形式はすべて OpenMPT の再生エンジン（libopenmpt）で実際に再生し、MOD と同じ音高・長さで鳴ることを自動テストで確認しています。
- **テンポ指定（`--tempo`）**:
...
| `cool` | – | 6 | 涼しげ。透明感のあるシンセと軽い2ステップのビート |
| `dark-tense` | – | 6 | 緊張感。低音のオスティナートと刻むパルス、重い打撃 |
| `dreamy` | – | 6 | 夢見心地。深い残響感のアルペジオとパッド |
| `energetic` | – | 6 | 元気・活動的。速いテンポと強いドラム、8分で刻むギターとベース |
| `focus` | – | 4 | 集中。ほとんど変化しないローファイのループと一定のテンポ |
| `melancholic` | – | 4 | 物悲しい。短調のピアノが旋律を歌い、弦のパッドが支える |
| `uplifting` | – | 6 | 上げていく高揚感。4つ打ちとアルペジオ、明るいスーパーソウのコード |
| `warm` | – | 4 | 温かい。アコースティックギターとピアノ、長調の穏やかな伴奏 |
...
| `bossa-nova` | – | 4 | ボサノバ。2/4 の柔らかいガットギターと軽いパーカッション |
| `cinematic` | – | 8 | 映画音楽。ピアノのオスティナートから弦とホルンが重なり、ドラマチックに高まる |
| `city-pop` | – | 6 | シティポップ。テンションコードのエレピ、跳ねるベースとギターのカッティング |
| `classical` | – | 4 | クラシック。古典派のメヌエット風の弦楽四重奏、明確な和声と終止 |
| `edm` | – | 6 | EDM。シンセ主体、ビルドアップで溜めてドロップで弾ける |
| `folk` | – | 4 | フォーク。アコースティックギターのストロークとフィドル、素朴な進行 |
| `free-jazz` | – | 4 | フリージャズ。トーンクラスター、確率密度のテクスチャ、ルバート（連続テンポ変化） |
| `future-bass` | – | 4 | フューチャーベース。キック連動サイドチェイン、ヴォーカルチョップ、Eb I-V-vi-IV |
| `hiphop` | – | 4 | ヒップホップ。ラップが乗る余白を残したブーンバップのビートとサンプル風ループ |
| `house` | – | 6 | ハウス。4つ打ちの安定したグルーヴと裏拍のオルガン・スタブ |
| `jazz` | – | 4 | ジャズ。ドリアンのモーダルなヴァンプ、4度堆積のピアノとミュート・トランペット |
| `lofi-hiphop` | – | 6 | ローファイ・ヒップホップ。よれたビート、ジャジーなエレピ、レコードのノイズ |
...
| `pop` | – | 6 | ポップ。長調の明るいメロディとピアノ、覚えやすいサビ |
| `prog-rock` | – | 4 | 変拍子プログレ／マスロック。7/8+7/8+5/8 のリフ、4/4 のコーラスとの対比 |
| `rnb-soul` | – | 6 | R&B／ソウル。滑らかなテンションコードと歌うような旋律のスロー・ジャム |
| `rock` | – | 6 | ロック。ギターのリフと8ビート、4/4 の中〜速いテンポ |
| `swing-jazz` | – | 4 | スウィング・ジャズ。ライド＋ウォーキングベース＋ピアノコンピング、Bbリズムチェンジ AABA |
| `synthwave` | – | 6 | シンセウェイブ。80年代のシンセとゲートスネア、8分で脈打つベース |
...
| ジャンル id | 別名 | ch | 説明 |
|:---|:---|:---|:---|
| `acoustic-ssw` | – | 4 | 弾き語り風。指弾きのギターと軽いパーカッション、歌のような旋律 |
| `ambient-drone` | – | 4 | ドローン。長く伸びる持続音がゆっくり移ろう、変化の少ない響き |
| `anime-ost` | – | 6 | アニメ劇伴風。刻むストリングスとジャズの和声、ブラスの決め |
| `indie-rock` | – | 6 | インディー・ロック風。生音のドラムと鳴り響くギターのアルペジオ、軽い歪み |
| `jpop-80s` | – | 6 | 80年代 J-POP 風。明るいコードと都会的なブラス、軽快なビートと最後のサビの転調 |
| `jrock-90s` | – | 6 | 90年代 J-ROCK 風。歪んだギターが前に出る速いビートとギターソロ、最後のサビで転調 |
| `jrpg` | – | 6 | ゲーム音楽風（JRPG）。旋律を重視した冒険のテーマ、ハープと弦とホルン |
| `lofi-chill` | – | 6 | ローファイ・チル。柔らかいギターとフルート、うねるサイドチェインと雨音 |
| `neo-soul` | – | 6 | ネオソウル風。よれたビートとエレピ主体の豊かなテンションコード |
...
| `trailer` | – | 8 | 映画予告編風。大太鼓と金管の衝撃、刻む弦、合唱で盛り上がる3幕構成 |
